/*=============================================================================
  ...
  Chạy trên: <PUBLISHER_HOST> (Máy chủ phát hành) sử dụng chế độ SQLCMD
  Mục đích : Điều phối các tập lệnh SQL theo đúng thứ tự.

  DB Máy chủ phát hành: NGANHANG_PUB

  Điều kiện tiên quyết (chạy TRƯỚC tập lệnh này):
    1. Tất cả bốn phiên bản SQL Server đang chạy:
         - <PUBLISHER_HOST>            (mặc định / Máy chủ phát hành)
         - <PUBLISHER_HOST>\SQLSERVER2 (CN1)
         - <PUBLISHER_HOST>\SQLSERVER3 (CN2)
         - <PUBLISHER_HOST>\SQLSERVER4 (TraCuu)
  ...
  Cách sử dụng (SQLCMD từ PowerShell / cmd):
    sqlcmd -S <PUBLISHER_HOST> -E -i "sql\99_run_all.sql"
...
/* =========================================================================
  GIAI ĐOẠN 1 — Thiết lập Máy chủ phát hành (chạy trên <PUBLISHER_HOST>)
   ========================================================================= */
...
PRINT '';
PRINT '──────────────────────────────────────────────────────';
PRINT 'Step 4b/8: Seed demo data (branches, employees,';
PRINT '           customers, accounts, transactions, logins)';
PRINT '           Idempotent — skips rows that already exist';
PRINT '──────────────────────────────────────────────────────';
:r .\04b_publisher_seed_data.sql
...
PRINT '          IMPORTANT: Run Step 7 on all subscribers BEFORE Step 5.';
PRINT '──────────────────────────────────────────────────────';
PRINT '>>> Step 5 is intentionally NOT auto-run in 99_run_all.sql to enforce correct order.';
PRINT '>>> Run this manually after Step 7:';
PRINT '    sqlcmd -S "<PUBLISHER_HOST>" -E -i "sql\05_replication_setup_merge.sql"';
