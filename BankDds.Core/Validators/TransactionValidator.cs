...
namespace BankDds.Core.Validators
{
    ...
    public class TransactionValidator : AbstractValidator<Models.Transaction>
    {
        public TransactionValidator()
        {
            ...
            // Logic: Source account must follow fixed SOTK format because transaction SPs rely on this key.
            RuleFor(x => x.SOTK)
                .NotEmpty().WithMessage("Số tài khoản (SOTK) là bắt buộc.")
                .Length(9).WithMessage("Số tài khoản phải đúng 9 ký tự.");

            // Logic: Only supported transaction types are accepted in business flow.
            RuleFor(x => x.LOAIGD)
            ...
            // Logic: Destination account is mandatory only for transfer transactions.
